#!/usr/bin/env python3
"""AgentStrategyV2 回测对比 — 多周期多策略横向比较"""
...
def load_config():
    path = os.path.join(os.path.dirname(__file__), "breadfree", "config.yaml")
...
def run(cls, symbols, sd, ed, cash, ds, label, **kw):
    ...
    engine = BacktestEngine(
        strategy_cls=cls, symbols=symbols, start_date=sd, end_date=ed,
    ...
    wr, _, tt = calculate_win_rate(tr)
    plr = calculate_profit_loss_ratio(tr)

    print(f"  净值 ¥{fe:,.2f} | 收益 {ret:.2%} | 年化 {ann:.2%} | "
          f"Sharpe {sh:.2f} | 回撤 {mdd:.2%} | 耗时 {elapsed:.1f}s")
    return {
        "label": label, "final_equity": fe, "total_return": ret,
        "annualized": ann, "sharpe": sh, "calmar": cal, "max_dd": mdd,
        "win_rate": wr, "trades": tt, "plr": plr, "time": elapsed,
    }
...
def main():
    ...
    for plabel, sd, ed in periods:
        ...
        r = run(RotationStrategy, syms, sd, ed, cash, ds,
                f"Rotation效率轮动 lb20n3 [{plabel}]",
                lookback_period=20, hold_period=20, top_n=3, use_efficiency=True)
        if r:
            results.append(r)

        r = run(AgentStrategyV2, syms, sd, ed, cash, ds,
                f"AgentV2(Bull-Bear辩证) [{plabel}]",
                lookback_period=20, hold_period=20, top_n=3)
        if r:
            results.append(r)

        r = run(EffiAgentRotationStrategy, syms, sd, ed, cash, ds,
                f"EffiAgent(LLM效率轮动) [{plabel}]",
    ...
    if results:
        ...
        fmt.columns = ["策略", "净值", "总收益", "年化", "Sharpe",
                        "Calmar", "最大回撤", "胜率", "交易", "盈亏比", "耗时"]
